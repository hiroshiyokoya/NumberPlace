...
class Board:
    ...
    def draw(self, init=False):
        if init:
            ...
            for row, col, val in zip(
                self.__cell_df.row, self.__cell_df.col, self.__cell_df.val
            ):
                board_arr[row - 1, col - 1] = val
    ...
    def list_candidates(self):
        return [
            cell.candidates if len(cell.candidates) > 0 else 0 for cell in self.__cells
        ]
...
def find_unique_candidates(cells):
    ...
    for cell in cells:
        ...
        union_other_candidates = set(
            sum([list(other.candidates) for other in cells if other.id != cell.id], [])
        )
        unique_candidate = [
            candidate
            for candidate in candidates
            if candidate not in union_other_candidates
        ]
    ...
    for cell in cells:
        ...
        if len(candidates) > 1:
            same_others = [
                other.id
                for other in cells
                if other.id != cell.id and candidates == other.candidates
            ]
            if len(same_others) == len(candidates) - 1:
                unique_pairings.append(
                    {"indices": set([cell.id] + same_others), "candidates": candidates}
                )
